...
final class WorkspaceModel: ObservableObject {
    ...
    private var connectedBrowsers: Set<String> = []
    ...
    func start() {
        ...
        if let watchedFolder { watch(URL(fileURLWithPath: watchedFolder)) }
    }
    func stop() {
        generation += 1; timer?.invalidate(); timer = nil
        watcherGeneration += 1; watcher = nil
        connectedBrowsers.removeAll(); browserObjects = []; browserStatus = [:]
    }
    func refresh() {
        guard !paused, !refreshing else { return }
        refreshing = true
        let gen = generation
        ...
        Task {
            let result = await Task.detached(priority: .utility) {
                ...
                if permitted { objects += WorkspaceSources.windows(snapshots) }
            ...
            let previousApps = objects.filter { $0.source == "macOS running applications" }
            if !previousApps.isEmpty {
                let previousIDs = Set(previousApps.map(\.id))
                let currentApps = result.1.filter { $0.source == "macOS running applications" }
